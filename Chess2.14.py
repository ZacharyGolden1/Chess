...
#  -------> have heuristic function done by 12/03
#  -------> done 12/05
...
def mouseReleased(app,event):
    ...
    movePiece(app)
    legalPieceMoves(app)
    print(generateAllFutureBoardStatesAndHeuristics(app,app.pieces,app.allLegalPieceMoves,'white'))
    if app.randomMode:
    ...
    if isInBoard(newrow,newcol):
        ...
        if piece in kings:
            for notKing in range(len(app.allLegalPieceMoves)):
                if app.allLegalPieceMoves[notKing] != None and\
                     app.allLegalPieceMoves[notKing] != 'wKing' and\
                     app.allLegalPieceMoves[notKing] != 'bKing':
                    if (newrow,newcol) in app.allLegalPieceMoves[notKing][1]:
...
def randomBlackPieceOnBoard(app):
    tempAllLegalPieceMoves = myDeepCopy(app.allLegalPieceMoves)
...
#*********** Minimax Attempt 1 *************#
# inspired by: https://www.chessprogramming.org/Evaluation
...
def generateOneFutureBoardState(app,pieces,moves,colorToMove):
    piecesCopy = copy.deepcopy(pieces)
    for move in moves:
        if move[0] != None:
            app.doableMoves.append(move)

    i = 0
    while i < len(app.doableMoves):
        if app.doableMoves[i][1] == []:
            app.doableMoves.pop(i)
        else:
            i += 1
    
    piecesCopy[app.doableMoves[0][0]][0] = app.doableMoves[0][1][0][0]
    piecesCopy[app.doableMoves[0][0]][1] = app.doableMoves[0][1][0][1]
    app.doableMoves.pop(0)
    return piecesCopy

def generateAllFutureBoardStatesAndHeuristics(app,pieces,moves,colorToMove):
    boardStates = []
    piecesCopy = copy.deepcopy(pieces)
    doableMoves = []
    allHeuristics = []
    ...
    i = 0
    while i < len(piecesCopy):
        piecesCopy[doableMoves[i][0]][0] = doableMoves[i][1][0][0]
        piecesCopy[doableMoves[i][0]][1] = doableMoves[i][1][0][1]
        allHeuristics.append(boardWorthHeuristic(app,piecesCopy))
        boardStates.append(piecesCopy)
        piecesCopy = copy.deepcopy(pieces)
        i+=1

    maxie,minie = max(allHeuristics),min(allHeuristics)
    maxPieces = boardStates[allHeuristics.index(max(allHeuristics))]
    minPieces = boardStates[allHeuristics.index(min(allHeuristics))]
        
    return maxie,minie,boardStates

def minimaxAlgo(app):
    pieces = copy.deepcopy(app.pieces)
    moves = myDeepCopy(app.allLegalPieceMoves)
    bestLine = 0
    minimaxHelper(app,pieces,moves,bestLine,'black',0)

def minimaxHelper(app,pieces,moves,bestLine,colorToMove,depth):
    legalAIPieceMoves(app,pieces)
    piece = generateOneFutureBoardState(app,pieces,moves,colorToMove)
    if colorToMove == 'black': 
        colorToMove == 'white'
    if colorToMove == 'white':
        colorToMove == 'black'
    if depth == 3:
        return boardWorthHeuristic(app,pieces)
    else:
        if colorToMove == 'black':
            return minimaxHelper(app,pieces,app.allLegalAIPieceMoves,bestLine,colorToMove,depth+1)[1]
        elif colorToMove == 'white':
            return minimaxHelper(app,pieces,app.allLegalAIPieceMoves,bestLine,colorToMove,depth+1)[0]
...
def AIMoveIsLegal(app,piece,newrow,newcol,olerow,olecol,pieces):
    ...
    if isInBoard(newrow,newcol):
        if piece in app.blackPieces and getAIPieceFromCell(app,newrow,newcol) != piece and\
            getAIPieceFromCell(app,newrow,newcol) in app.blackPieces or\
            piece in app.whitePieces and getAIPieceFromCell(app,newrow,newcol) != piece and\
            getAIPieceFromCell(app,newrow,newcol) in app.whitePieces:
            if piece not in knights:
                if newrow > olerow and newcol == olecol:
                    for row in range(newrow,8):
        ...
        if piece in app.blackPieces and getAIPieceFromCell(app,newrow,newcol) != piece and\
            getAIPieceFromCell(app,newrow,newcol) in app.whitePieces or\
            piece in app.whitePieces and getAIPieceFromCell(app,newrow,newcol) != piece and\
            getAIPieceFromCell(app,newrow,newcol) in app.blackPieces:
            if piece not in knights:
                if newrow > olerow and newcol == olecol:
                    for row in range(newrow+1,8):
                        app.afterAIPieceRowColDiag.append((row,olecol))
        ...
        if piece in kings and\
            ...
            return True
        if piece in kings and\
            (app.canAIWKingCastle and app.canAIlWRookCastle and getAIPieceFromCell(app,7,1) == 0 and\
            getAIPieceFromCell(app,7,2) == 0 and getAIPieceFromCell(app,7,3) == 0 and\
            newcol - olecol == -2 and newrow == olerow and piece == kings[0]):
            return False
        if piece in kings and\
            (app.canAIWKingCastle and app.canAIrWRookCastle and getAIPieceFromCell(app,7,5) == 0 and\
            getAIPieceFromCell(app,7,6) == 0 and newcol - olecol == 2 and\
            newrow == olerow and piece == kings[0]):
            return False
        if piece in kings and\
            (app.canAIBKingCastle and app.canAIlBRookCastle and getAIPieceFromCell(app,0,1) == 0 and\
            getAIPieceFromCell(app,0,2) == 0 and getAIPieceFromCell(app,0,3) == 0 and\
            newcol - olecol == -2 and newrow == olerow and piece == kings[1]):
            return True
        if piece in kings and\
            (app.canAIBKingCastle and app.canAIrBRookCastle and getAIPieceFromCell(app,0,5) == 0 and\
            getAIPieceFromCell(app,0,6) == 0 and newcol - olecol == 2 and\
            newrow == olerow and piece == kings[1]):
            return True
        elif piece in queens and\
