...
def generateAllFutureBoardStates(app,pieces,moves,colorToMove):
    boardStates = []
    piecesCopy = copy.deepcopy(pieces)
    doableMoves = []
    for move in moves:
    ...
    if colorToMove == 'white':   
        while i < len(piecesCopy):
            if doableMoves[i][0] in app.whitePieces:
    ...
    if colorToMove == 'black':   
        while i < len(doableMoves):
            if doableMoves[i][0] in app.blackPieces:
                piecesCopy[doableMoves[i][0]][0] = doableMoves[i][1][0][0]
                piecesCopy[doableMoves[i][0]][1] = doableMoves[i][1][0][1]
            boardStates.append(piecesCopy)
            piecesCopy = copy.deepcopy(pieces)
...
def minimaxHelper(app,pieces,moves,bestLine,colorToMove,depth):
    print('   '*depth,bestLine)
    legalAIPieceMoves(app,pieces)
    if colorToMove == 'black': 
        colorToMove == 'white'
    if colorToMove == 'white':
        colorToMove == 'black'
    boardStates = generateAllFutureBoardStates(app,pieces,moves,colorToMove)
    if depth == 3:
        return bestLine
    else:
        if colorToMove == 'black':
            for state in boardStates:
                if bestLine > boardWorthHeuristic(app,state):
                    bestLine = boardWorthHeuristic(app,state)
                minimaxHelper(app,state,app.allLegalAIPieceMoves,bestLine,colorToMove,depth+1)
