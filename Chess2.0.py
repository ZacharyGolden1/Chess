...
def isBeforePiece(row,col,pieceRow,pieceCol):
    if -1<row<=pieceRow and -1<col<=pieceCol:
        return True
    else: return False
...
def movePiece(app):
    ...
    for piece in app.pieces:
        if app.pieces[piece] == elementsOfPiece:
            ...
            if not moveIsLegal(app,piece,newrow,newcol,olerow,olecol):
...
def moveIsLegal(app,piece,newrow,newcol,olerow,olecol):
    ...
    if isInBoard(newrow,newcol):
        if piece in app.blackPieces and getPieceFromCell(app,newrow,newcol) != piece and\
            ...
            piece in app.blackPieces and getPieceFromCell(app,newrow,newcol) != piece and\
            getPieceFromCell(app,newrow,newcol) in app.whitePieces or\
            piece in app.whitePieces and getPieceFromCell(app,newrow,newcol) != piece and\
        ...
        if piece in app.blackPieces and getPieceFromCell(app,newrow,newcol) != piece and\
            ...
            return True
        elif piece in bishops and\
            ((newcol - olecol == newrow - olerow) or\
             (newcol - olecol == -(newrow - olerow))):
            return True
        elif piece in rooks and\
            ((newrow - olerow == 0) or\
             (newcol - olecol == 0)):
            return True
        elif piece in pawns and piece in app.blackPieces and\
            (((newrow - olerow) == 1 and olecol - newcol == 0) or\
            (app.pieces[piece][0] == 1 and (newrow - olerow) == 2 and olecol - newcol == 0) or\
            ...
            return True
        elif piece in pawns and piece not in app.blackPieces and\
            (((newrow - olerow) == -1 and olecol - newcol == 0) or\
            (app.pieces[piece][0] == 6 and (newrow - olerow) == -2 and olecol - newcol == 0) or\
            (getPieceFromCell(app,newrow,newcol) != 0 and\
...
def pieceCaptured(app,x,y,xx,yy):
    ...
    if moveIsLegal(app,oldPiece,newRow,newCol,oleRow,oleCol):
        if not selectPiece(app):
            pass
#            if oldPiece in app.blackPieces and newPiece not in app.blackPieces or\
#                oldPiece not in app.blackPieces and newPiece in app.blackPieces:
#                if newPiece in app.blackPieces:
#                    app.pieces[newPiece][0] == 8
#                else:
#                    app.pieces[newPiece][0] == -1
